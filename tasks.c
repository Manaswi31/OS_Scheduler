...
int i = 0, executionTime = 0, quantum = 10;
...
node *sigNode;
sigNode = p;
tidScheduler = pthread_self();
printf("Scheduler task id : %ld\n", tidScheduler);
...
sigfillset(&sigmask);                /* will unblock all signals */  
  pthread_sigmask(SIG_UNBLOCK, &sigmask, (sigset_t *)0);
   ...
   sigwait(&sigmask, &sig);
printf("Waiting for signal from threads\n");
}
